...
object WebSocketFlow {
  ...
  def actorRef[In, Out](
    ...
    logger.debug("-=-=-=Create new channel by WebSocketFlow=-=-=-")
    val channel =
    ...
    logger.debug(s"WebSocket Channel :: $channel")
    val sink = Sink.actorRef(channel, Status.Success(()), (t: Throwable) => Status.Failure(t))
    logger.debug("-=-=-=End create channel by WebSocketFlow=-=-=-")
  ...
  private def channelActor(props: ActorRef => Props, out: ActorRef): Actor = new Actor {
    ...
    def receive: Receive = {
      case Status.Success(_) | Status.Failure(_) => logger.debug("--PoisonPill--" + sender());wsFlowActor ! PoisonPill
      case Terminated(_)                         => logger.debug("--Terminated--" + sender());context.stop(self)
      case util.Success(value)                   => logger.debug("--Forward received message--"); wsFlowActor ! value
      case _                                     => out ! NotificationProtocol.Notify("PONG") // 不符合消息协议的内容视为心跳检测
    }
